...
class AgentFisher(Agent):
    def __init__(self, agent_id, threshold, problem_id=None, ):
        ...
        self.flag_allocation_to_send = {}
        self.reset_flag_allocation_to_receive()
        self.flag_bids_receive = False
        self.flag_allocation_to_receive = {}
        self.flag_bids_receive = False
        self.reset_flag_allocation_to_send()
    ...
    def reset_flag_allocation_to_receive(self):
        for mission in self.mission_responsibility:
            self.flag_allocation_to_receive[mission] = False

    def reset_flag_allocation_to_send(self):
        for mission in self.mission_responsibility:
            self.flag_allocation_to_send[mission] = False
    ...
    def agent_receive_a_single_msg(self, msg):
        ...
        if isinstance(msg, MsgFisherBid):
            ...
            self.flag_bids_receive[mission] = True
        if isinstance(msg, MsgFisherAllocation):
            x_ij = msg.context
            self.x_i[msg.sender_id] = x_ij
            self.flag_allocation_to_receive = True
